...
def score_reviews_basic(state):
    ...
    print("---SCORING REVIEWS (BASE MODEL)---")
    documents = state["documents"]
    product = state["product"]
...
def query_advanced_rag_base(question):
    print("\n" + "="*80)
    print(f"ADVANCED RAG WITH BASE MODEL: {question}")
    print("="*80)
...
def query_advanced_rag_lora(question):
    print("\n" + "="*80)
    print(f"ADVANCED RAG WITH LORA FINE-TUNED MODEL: {question}")
    print("="*80)
...
# Define the 5 test questions
test_questions = [
    "What are the advantages and disadvantages of KIND Bars?",
    "Tell me about the pros and cons of Haagen-Dazs Ice Cream",
    "Tell me about the if I should use Starbucks brand coffee as my daily coffee",
    "What should I know if I want to buy San Pellegrino Sparkling Water for Party",
    "What brand of milk contains most nutritions and protein"
]

def save_results_to_file(results):
    with open("rag_evaluation_results.txt", "w") as f:
        for question_idx, question in enumerate(test_questions):
            f.write("="*80 + "\n")
            f.write(f"QUESTION {question_idx+1}: {question}\n")
            f.write("="*80 + "\n\n")
            
            # Base LLM results
            f.write("BASE LLM RESPONSE:\n")
            f.write("-"*40 + "\n")
            f.write(results[question_idx]["base_llm"] + "\n\n")
            
            # Basic RAG results
            f.write("BASIC RAG RESPONSE:\n")
            f.write("-"*40 + "\n")
            f.write(results[question_idx]["basic_rag"] + "\n\n")
            
            # Advanced RAG with base model
            f.write("ADVANCED RAG WITH BASE MODEL RESPONSE:\n")
            f.write("-"*40 + "\n")
            f.write(results[question_idx]["advanced_rag_base"] + "\n\n")
            
            # Advanced RAG with LoRA
            f.write("ADVANCED RAG WITH LORA FINE-TUNED MODEL RESPONSE:\n")
            f.write("-"*40 + "\n")
            f.write(results[question_idx]["advanced_rag_lora"] + "\n\n\n")
    
    print(f"Results saved to rag_evaluation_results.txt")
...
def main():
    ...
    for question_idx, question in enumerate(test_questions):
        ...
        print("-"*40)
        base_llm_response = query_base_llm(question)
        print(base_llm_response + "\n")
        
        # Basic RAG results
        print("BASIC RAG RESPONSE:")
        print("-"*40)
        basic_rag_response = query_basic_rag(question)
        print(basic_rag_response + "\n")
        
        # Advanced RAG with base model
        print("ADVANCED RAG WITH BASE MODEL RESPONSE:")
        print("-"*40)
        advanced_rag_base_response = query_advanced_rag_base(question)
        print(advanced_rag_base_response + "\n")
        
        # Advanced RAG with LoRA
        print("ADVANCED RAG WITH LORA FINE-TUNED MODEL RESPONSE:")
        print("-"*40)
        advanced_rag_lora_response = query_advanced_rag_lora(question)
        print(advanced_rag_lora_response + "\n\n")
